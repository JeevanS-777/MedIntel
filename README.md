...
3. Decode JWT with JWT_SECRET
4. Verify token signature & expiration
5. Inject req.user with decoded payload
6. Pass to route handler or reject with 401
```

### Security Features
- ✅ **Password Hashing**: bcryptjs with salt rounds 10
- ✅ **JWT Tokens**: 7-day expiration, signed with secret
- ✅ **File Upload Validation**: Only JPEG/PNG/PDF, max 15MB
- ✅ **CORS**: Configured for frontend domain
- ✅ **SQL Injection Prevention**: Parameterized queries with mysql2
- ✅ **Environment Variables**: Sensitive data in .env
- ✅ **Route Protection**: All sensitive endpoints require auth middleware
...
# Or use PM2:
npm install -g pm2
pm2 start server.js --name "medintel-backend"
```
...
3. Ensure database "medintel" exists
...
```
Solution:
1. Install Tesseract from https://github.com/UB-Mannheim/tesseract
2. Update pytesseract.pytesseract.tesseract_cmd path in app.py
...
```
Solution:
1. Verify backend/server.js has CORS enabled
2. Check frontend URL in CORS whitelist
3. Ensure proper headers in axios requests
```
...
### Flask AI Service Connection Failed
```
Solution:
1. Ensure Flask is running on http://localhost:5001
2. Check AI_SERVICE_URL in backend/.env
3. Verify CORS enabled in Flask app
```
...
const res = await axios.post(
    'http://localhost:5000/api/new/new-endpoint',
    data,
...
The AI service monitors for emergency keywords:
- "heart attack"
- "stroke"
- "severe chest pain"
- "emergency"
...
When detected, the system flags the response with elevated alert status for medical professionals.
...
- ✅ Connection pooling (10 connections, MySQL2)
- ✅ Keep-alive enabled with 10s timeout
- ✅ Pagination for large report lists
- ✅ Indexed database columns (email, user_id)

### Frontend Optimizations
- ✅ Vite fast bundling & code splitting
- ✅ Lazy loading of routes
- ✅ Image optimization via Tailwind
- ✅ localStorage caching of JWT tokens

### AI Service Optimizations
- ✅ Sentence-Transformers caching (all-MiniLM-L6-v2)
